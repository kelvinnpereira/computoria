const SolicitacoesRecebidas = ({ usuario, agenda, aceitar, recusar }) => {
  const semanas = [
    'Domingo',
    'Segunda-Feira',
    'Terça-Feira',
  ...
  const buttonAceitar = (event) => {
    aceitar({ id: event.target.id });
  }

  const buttonRecusar = (event) => {
    recusar({ id: event.target.id });
  }

  return (
    <div className="w-full items-center">
      {agenda?.map((item) => {
        if (item.status === 'solicitada' && item.matricula_tutor === usuario.matricula) {
            ...
            <div className='w-1/3 widget p-4 mb-4 rounded-lg bg-white border border-gray-100 dark:bg-gray-900 dark:border-gray-800'>
              <div className="flex items-center">
                ...
                <div className="pl-4">
                  ...
                  <a href={`/perfil/${item.matricula_aluno}`}><b>{item.nome_aluno}</b></a> mandou uma solicitação de agendamento de ajuda na disciplina <b>{`${item.sigla_disciplina} - ${item.disciplina}.`}</b>
                  </p>
