...
const Aulas = ({ usuario, agenda, cancelar }) => {
  const semanas = [
    'Domingo',
    'Segunda-Feira',
    'Terça-Feira',
  ...
  return (
    <div className="w-full items-center">
      {agenda?.map((item) => {
        if (item.status === 'agendada' && item.matricula_aluno === usuario.matricula) {
              ...
              <div className="flex items-center">
                ...
                <div className="pl-4">
                  <p className="text-lg font-bold">
                    Você tem uma aula de <b>{`${item.sigla_disciplina} - ${item.disciplina}`}</b> marcada com o tutor/monitor <a href={`/perfil/${item.matricula_tutor}`}><b>{item.nome_tutor}</b></a>.
                  </p>
