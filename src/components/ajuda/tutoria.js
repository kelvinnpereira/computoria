...
const Tutorias = ({ usuario, agenda, cancelar }) => {
  const semanas = [
    'Domingo',
    'Segunda-Feira',
    'Terça-Feira',
  ...
  const buttonReagendar = (event) => {
    Router.push('/ajuda/tutor_reagendar/' + event.target.id);
  }
  ...
  return (
    <div className="w-full items-center">
      {agenda?.map((item) => {
        if (item.status === 'agendada' && item.matricula_tutor === usuario.matricula) {
              ...
              <div className="flex items-center">
                ...
                <div className="pl-4">
                  <p className="text-lg font-bold">
                    Você tem uma aula de <b>{`${item.sigla_disciplina} - ${item.disciplina}`}</b> marcada com o aluno <a href={`/perfil/${item.matricula_aluno}`}><b>{item.nome_aluno}</b></a>. 
                  </p>
