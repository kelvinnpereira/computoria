...
import { useRequest, useCsrf } from "../../hooks/auth";

const AddDisciplinas = (data) => {
  ...
  const [isLoading, setRequest] = useRequest(onSignup, onError, '/proficiencia');
  ...
  return (
    <Modal title={'Disciplinas'} body={body()} open={showModal} setOpen={setModal} btns={buttonModal()} />
  )
}
...
export const getServerSideProps = async (context) => {
  const proficiencia = await get('/proficiencia');
  const improficiencia = await get('/improficiencia');
  return {
    props: {
      data: {
        proficiencia: proficiencia.data,
        improficiencia: improficiencia.data
      }
    },
  }
}